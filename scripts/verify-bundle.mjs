...
if (!bundle.includes("Glass Tube Display Card v0.3.10")) fail("root bundle version is not v0.3.10");
...
if (!renderer.includes('src="${SCREW_ASSET_URI}"')) fail("screw renderer does not use the exact Gauge screw asset");
...
if (!styles.includes(".screw{position:absolute;z-index:20;width:clamp(58px,8.2vw,88px)")) fail("Gauge screw crop is not displayed at the correct responsive scale");
if (!styles.includes("object-fit:contain;filter:none")) fail("Gauge screw artwork is being altered by CSS filters");
...
if (!core.includes('const VERSION = "0.3.10"')) fail("card-core version is not v0.3.10");
...
if (!process.exitCode) console.log("Glass Tube Display Card v0.3.10 verification passed.");
